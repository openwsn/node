...
* LOCAL VARIABLES
*/
static ISR_FUNC_PTR joystick_isr_tbl[HAL_JOYSTICK_MAX_EVENTS] = {0};
...
uint8 halJoystickIntConnect(uint8 event, ISR_FUNC_PTR func)
{
    ...
    HAL_INT_LOCK(key);
    switch(event) {
    case HAL_JOYSTICK_UP:
        joystick_isr_tbl[HAL_JOYSTICK_UP] = func;
        break;
    case HAL_JOYSTICK_DOWN:
        joystick_isr_tbl[HAL_JOYSTICK_DOWN] = func;
        break;
    case HAL_JOYSTICK_LEFT:
        joystick_isr_tbl[HAL_JOYSTICK_LEFT] = func;
        break;
    case HAL_JOYSTICK_RIGHT:
        joystick_isr_tbl[HAL_JOYSTICK_RIGHT] = func;
        break;
    case HAL_JOYSTICK_PUSHED:
        joystick_isr_tbl[HAL_JOYSTICK_PUSHED] = func;
        break;
    default:
...
uint8 halJoystickGetDir(void)
{
    ...
    if (value > 0x0B00)
        return HAL_JOYSTICK_CENTER;
    if (value > 0x0900)
        return HAL_JOYSTICK_RIGHT;
    if (value > 0x0800)
        return HAL_JOYSTICK_LEFT;
    if (value > 0x0600)
        return HAL_JOYSTICK_DOWN;
    if (value > 0x0100)
        return HAL_JOYSTICK_UP;

    return HAL_JOYSTICK_INVALID;
}
...
static void halJoystickMoveISR(void)
{
    ...
    uint8 pushed    = JOYSTICK_PUSHED();

    if (pushed)
    {
        if(joystick_isr_tbl[HAL_JOYSTICK_PUSHED] != 0)
            (*joystick_isr_tbl[HAL_JOYSTICK_PUSHED])();
    }
    else if (direction & JOYSTICK_UP_BM)
    {
        if(joystick_isr_tbl[HAL_JOYSTICK_UP]  != 0)
            (*joystick_isr_tbl[HAL_JOYSTICK_UP])();
    }
    else if (direction & JOYSTICK_DOWN_BM)
    {
        if(joystick_isr_tbl[HAL_JOYSTICK_DOWN]  != 0)
            (*joystick_isr_tbl[HAL_JOYSTICK_DOWN])();
    }
    else if (direction & JOYSTICK_LEFT_BM)
    {
        if(joystick_isr_tbl[HAL_JOYSTICK_LEFT]  != 0)
            (*joystick_isr_tbl[HAL_JOYSTICK_LEFT])();
    }
    else if (direction & JOYSTICK_RIGHT_BM)
    {
        if(joystick_isr_tbl[HAL_JOYSTICK_RIGHT]  != 0)
            (*joystick_isr_tbl[HAL_JOYSTICK_RIGHT])();
    }
}
