...
/*
uint8_t cc2420_writeframe( char * buf, uint8 len )
{
   cc2420_writetxfifo( len, buf );
   if success 
      return len;
    else 
	  return -1;
...
/*********************added by zw*********************
uint8_t cc2420_readframe(uint8_t * msg, uint8_t size )
{
    uint8_t num,i,flag;
	...
	// copy the frame in cc2420_rxbuf_now[] to buf
	// memmove or for loop
/*	
	usart_putbyte(0x7e);
	usart_putbyte(0x42);
...
*/	
/*
	free(cc2420_rxbuf[flag]);// flag: the current frame
	cc2420_rxbufuse--;  // how many frame in the buffer
...
/**********************20090516***********************/
uint8_t cc2420_sendframe( uint8_t * msg, uint8_t len )
{
	uint8_t i;
	//uint8_t buf[127];
	for(i=0;i<len;i++)
	...
	uint8_t cc2420_writetxfifo(len, &buf[0]);
}
/*****************************************************/
inline uint8_t cc2420_receiveframe(uint8_t *msg)
{
  uint8_t status;
  uint8_t i;
  //uint8_t flag;
 ...
 // printf("***开始一个读fifo任务***\n");
  { hal_atomic_t _nesc_atomic = hal_atomic_start();
      ...
      if (cc2420_rxlength > 0) 
	{
            ...
            //len=cc2420_rxlength;//added
			//cc2420_rxlength ++;
  
            for (i = 0; i < cc2420_rxlength ; i++)
	     {
                SPDR = 0;
                while (!(SPSR & 0x80));
                buf[i] = SPDR;//读出MAC帧
