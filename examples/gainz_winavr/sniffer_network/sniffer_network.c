...
/** 
 * Networked Sniffer System(nss). 
 * @return None.
 */
void nss_execute(void)
{
	char * msg = "welcome to networked sniffer system ...";
    TiCc2420Adapter * cc;
    TiFrame * nio_rxbuf;
	...
	while(1) 
	{
		...
		// len = uart_read( uart, sio_rxbuf, SIO_RXBUF_SIZE, 0x00 );
		...
		if (len > 0)
		{
            uart_putchar( uart, 0xff);//todo for testing
			//todo 从串口端发送地址的时候第一位是低位第二位是高位
			//目前发送指令应为 05 00 01
            //uart_write( uart,sio_rxbuf,len,0x00);//todo for testing
			sio_addr = ((uint16)(sio_rxbuf[1] << 8) | (uint16)(sio_rxbuf[0]));
			if (sio_addr == LOCAL_ADDRESS) 
			{
				// uart_putchar( uart, 0xfd);//todo for testing
				switch (sio_rxbuf[2])
				{
				case CMD_DATA_REQUEST:
					//uart_putchar( uart, 0xfc);//todo for testing
...
void nss_send_response( TiFrameQueue * fmque, TiSnifferStatistics * stat, TiUartAdapter * uart )
{
	...
	f = fmque_front( fmque );
	//uart_putchar( uart,0xfa);//todo for testing
	while (f != NULL)
	{
		//uart_putchar( uart,0xfb);//todo for testing
		uart_write( uart,frame_startptr(f),frame_length(f),0x00);//todo for testing
